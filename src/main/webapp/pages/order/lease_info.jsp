	...
	if(buuid != ""){
		$.post("${path}/position/office/getOfficeByBuilding.json", {
			...
			for(var i=0;i<data.length;i++){
				...
				$("#offuuid").append("<option value='"+data[i].uuid+"'>"+type+data[i].code+"</option>");
			}
		});
	}
}
			...
			<select id="offuuid" name="office.uuid" class="form-control input-sm" data-rule-required="true">
					<option value=''>请选择</option>
                     <c:forEach items="${officelist}" var="office">
                     		<c:choose>
                     			<c:when test="${office.uuid == reservation.office.uuid}"><option value="${office.uuid}" selected="selected">
                     				<c:choose>
                     			...
                     			<c:otherwise><option value="${office.uuid}"><c:choose>
                     					<c:when test="${office.type==0}">（办公桌）</c:when>
                     					<c:otherwise>（办公室）</c:otherwise>
                     				</c:choose>${office.code}</option></c:otherwise>
                     		</c:choose>
