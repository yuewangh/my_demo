...
<head>
    ...
    $(function () {
    	...
    	data_url = "${path}/order/lease/getList.json";
    	$(grid_selector).jqGrid({
    		url: data_url,
            datatype: "json",
            height: 220,
            colNames: [ 'totalPrice','uuid','预订号', '申请人','办公楼', '办公室/桌','编号', '开始日期', '结束日期', '是否成交',  '成交金额（元）'],
            colModel: [
                {name: 'totalPrice', index: 'totalPrice',hidden: true},
                {name: 'uuid', index: 'uuid',hidden: true},
                {name: 'reservationNum', index: 'reservationNum', width: 150,
                	formatter:function(cellvalue, options, rowObject){
   					 return "<a href='${path}/order/lease/getDetail.vhtml?uuid="+rowObject['uuid']+"' style='color:blue'>"+cellvalue+"</a>";
   					}	
                },
                ...
                {name: 'finalPrice', index: 'finalPrice', width: 100,align:"center"}
            ],
            viewrecords: true,
            rowNum: 10,
            rowList: [10, 20, 30],
...
//                multiboxonly: true,
    		...
    		addfunc:function(rowid){
    			 $("#userInfoDiv_content").load("${path}/order/lease/toedit.action", function () {
    		...
    		editfunc:function(rowid){
    			var data = jQuery(grid_selector).jqGrid('getRowData', rowid);
    			 $("#userInfoDiv_content").load("${path}/order/lease/toedit.action?uuid=" + data.uuid, function () {
    	             $("#userInfoDiv").modal('show');
    	         });
    		},
    		del:true,
    	...
    	function deleteChartInfoBox(rowid){
 	       ...
 	       if(confirm("您确认要删除此订单吗？")){
				$.post("${path}/order/lease/delete.json", {
					"id" : data.uuid
				}, function(data) {
					if(data.success){
						alert("删除成功！");
...
<div>
                    <div class="panel panel-default">
                        ...
                        <div class="panel-heading">已成交订单列表</div>
