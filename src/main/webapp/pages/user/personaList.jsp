...
<script type="text/javascript" src="${path}/scripts/My97DatePicker/WdatePicker.js" ></script>
    ...
    $(function () {
    	...
    	data_url = "${path}/system/persona/list.json";
    	$(grid_selector).jqGrid({
            ...
            colModel: [
                ...
                {name: 'gender', index: 'gender', width: 50,formatter: genderFmatter,align:"center", sortable: false},
    		...
    		addtext:'新增',
    		addfunc:function(){
    			$("#userInfoDiv_content").load("${path}/system/persona/addOrupdateView.action", function () {
    		...
    		editfunc:function(rowid){
    			var data = jQuery(grid_selector).jqGrid('getRowData', rowid);
    			 $("#userInfoDiv_content").load("${path}/system/persona/addOrupdateView.action?uuid=" + data.uuid, function () {
    	                $("#userInfoDiv").modal('show');
    	            });
    		},
    		del:true,
    		...
    		delfunc:function(rowid){
    			...
    			if(ids.length > 1 || ids.length == 0 || typeof(ids) == "undefined"){
					...
					if(confirm("确定要删除选择的用户吗？")){
						...
						$.ajax({
		                    'url': '${path}/system/persona/delete.json',
		                    'type': 'get',
		                    'data': {
		                        uuid:data.uuid
		                    },
    	...
    	function toSearch(){
    		$(grid_selector).jqGrid('setGridParam', {postData: {loginName: $("#_loginName").val(),name:$("#_name").val(),survival:$("#_survival").val(),tel:$("#_tel").val()}}).trigger("reloadGrid");
    	}
    </script>
</head>
<body>
